...
{
  "name": "superfastpowers",
  "version": "$MANIFEST_VERSION"
}
EOF

    cat > "$repo/assets/superfastpowers-small.svg" <<'EOF'
...
EOF
    ...
    git -C "$repo" add \
        ...
        .gitignore \
        assets/app-icon.png \
        assets/superfastpowers-small.svg \
        package.json \
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/superfastpowers/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/superfastpowers/.fixture-keep"
    cat > "$repo/plugins/superfastpowers/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/superfastpowers/.fixture-keep
    git -C "$repo" add plugins/superfastpowers/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/superfastpowers/skills/example/agents"

    cat > "$repo/plugins/superfastpowers/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/superfastpowers/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/superfastpowers/.codex-plugin" \
        "$repo/plugins/superfastpowers/.private-journal" \
        "$repo/plugins/superfastpowers/assets" \
        "$repo/plugins/superfastpowers/skills/example/agents" \
        "$repo/plugins/superfastpowers/skills/example"

    cat > "$repo/plugins/superfastpowers/.codex-plugin/plugin.json" <<EOF
{
  "name": "superfastpowers",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/superfastpowers/assets/superfastpowers-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/superfastpowers/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/superfastpowers/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/superfastpowers/.codex-plugin/plugin.json \
        plugins/superfastpowers/assets/app-icon.png \
        plugins/superfastpowers/assets/superfastpowers-small.svg \
        plugins/superfastpowers/skills/example/agents/openai.yaml \
        plugins/superfastpowers/skills/example/SKILL.md \
        plugins/superfastpowers/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/superfastpowers/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/superfastpowers/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/superfastpowers/.private-journal/leak.txt"
    git -C "$repo" add plugins/superfastpowers/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/superfastpowers/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/superfastpowers/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/superfastpowers-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (superfastpowers-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/superfastpowers-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/superfastpowers/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/superfastpowers" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/superfastpowers-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/superfastpowers'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/superfastpowers-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/superfastpowers-*" "Clean no-op local apply does not create sync branch in destination checkout"
