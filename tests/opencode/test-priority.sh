...
# 1. Create in superfastpowers location (lowest priority)
mkdir -p "$SUPERFASTPOWERS_SKILLS_DIR/priority-test"
cat > "$SUPERFASTPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Superfastpowers version of priority test skill
---
# Priority Test Skill (Superfastpowers Version)

This is the SUPERFASTPOWERS version of the priority test skill.

PRIORITY_MARKER_SUPERFASTPOWERS_VERSION
EOF
...
if [ -f "$SUPERFASTPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Superfastpowers version exists"
else
    echo "  [FAIL] Superfastpowers version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled superfastpowers priority
echo ""
echo "Test 2: Documenting personal vs superfastpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superfastpowers version instead of personal version"

# Test 3: Document project vs bundled superfastpowers priority
echo ""
echo "Test 3: Documenting project vs personal/superfastpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superfastpowers version instead of project version"

# Test 4: Test a non-colliding bundled superfastpowers skill is still available
echo ""
echo "Test 4: Testing non-colliding superfastpowers skill remains available..."

mkdir -p "$SUPERFASTPOWERS_SKILLS_DIR/superfastpowers-only-test"
cat > "$SUPERFASTPOWERS_SKILLS_DIR/superfastpowers-only-test/SKILL.md" <<'EOF'
---
name: superfastpowers-only-test
description: Superfastpowers-only priority test skill
---
# Superfastpowers Only Test Skill

PRIORITY_MARKER_SUPERFASTPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"superfastpowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SUPERFASTPOWERS_ONLY_VERSION" "Non-colliding superfastpowers skill is still registered"
