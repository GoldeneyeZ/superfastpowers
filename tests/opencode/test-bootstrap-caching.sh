...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERFASTPOWERS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$SUPERFASTPOWERS_SKILLS_DIR/using-superfastpowers/SKILL.md" "$TEST_HOME/using-superfastpowers.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERFASTPOWERS_PLUGIN_FILE" missing
}
