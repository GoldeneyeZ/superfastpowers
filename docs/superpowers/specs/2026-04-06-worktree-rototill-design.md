...
For harnesses without native support (Codex CLI, OpenCode, Copilot standalone), superfastpowers fills a real gap. The skill shouldn't go away — it should get out of the way when native support exists.
...
- `.superfastpowers-session.json` metadata tracking (interesting PR #997 idea, not needed for v1)
...
Whoever creates the worktree owns its cleanup. If the harness created it, superfastpowers doesn't touch it. If superfastpowers created it (via git fallback), superfastpowers cleans it up. The heuristic: if the worktree lives under `.worktrees/` or `~/.config/superfastpowers/worktrees/`, superfastpowers owns it. Anything else (`.claude/worktrees/`, `~/.codex/worktrees/`, `.gemini/worktrees/`) belongs to the harness.
...
2. Check for existing `~/.config/superfastpowers/worktrees/<project>/` directory — if found, use it (backward compatibility with legacy global path).
...
No interactive directory selection prompt. The global path (`~/.config/superfastpowers/worktrees/`) is no longer offered as a choice to new users, but existing worktrees at that location are detected and used for backward compatibility.
...
git worktree remove "$WORKTREE_PATH"  # only if superfastpowers owns it
...
**Option 4 (Discard):** Require typed "discard" confirmation. Then remove worktree (if superfastpowers owns it), force-delete branch.
...
if worktree path is under .worktrees/ or ~/.config/superfastpowers/worktrees/:
    # Superfastpowers created it — we own cleanup
...
The `.worktrees/` or `~/.config/superfastpowers/worktrees/` = ours, anything else = hands off` heuristic works for every current harness. If a future harness adopts `.worktrees/` as its convention, we'd have a false positive (superfastpowers tries to clean up a harness-owned worktree). Similarly, if a user manually runs `git worktree add .worktrees/experiment` without superfastpowers, we'd incorrectly claim ownership. Both are low risk — every harness uses branded paths, and manual `.worktrees/` creation is unlikely — but worth noting.
