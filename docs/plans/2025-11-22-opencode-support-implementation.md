...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superfastpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add full superfastpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/superfastpowers-codex` (lines 40-74)
...
- Reference: `.codex/superfastpowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'superfastpowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superfastpowers-codex` (lines 212-280)
 ...
 * (personal skills override superfastpowers skills).
 *
 * @param {string} skillName - Name like "superfastpowers:brainstorming" or "my-skill"
 * @param {string} superfastpowersDir - Path to superfastpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superfastpowersDir, personalDir) {
    // Strip superfastpowers: prefix if present
    const forceSuperfastpowers = skillName.startsWith('superfastpowers:');
    const actualSkillName = forceSuperfastpowers ? skillName.replace(/^superfastpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly superfastpowers:)
    if (!forceSuperfastpowers && personalDir) {
    ...
    // Try superfastpowers skills
    if (superfastpowersDir) {
        const superfastpowersPath = path.join(superfastpowersDir, actualSkillName);
        const superfastpowersSkillFile = path.join(superfastpowersPath, 'SKILL.md');
        if (fs.existsSync(superfastpowersSkillFile)) {
            return {
                skillFile: superfastpowersSkillFile,
                sourceType: 'superfastpowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/superfastpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/superfastpowers-codex` (add import at top)
...
Run: `node -c .codex/superfastpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/superfastpowers-codex
...
**Files:**
- Modify: `.codex/superfastpowers-codex` (lines 40-74)
...
Run: `.codex/superfastpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/superfastpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/superfastpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/superfastpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/superfastpowers.js`
...
/**
 * Superfastpowers plugin for OpenCode.ai
 *
...
const superfastpowersSkillsDir = path.join(homeDir, '.config/opencode/superfastpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const SuperfastpowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/superfastpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/superfastpowers.js
...
**Files:**
- Modify: `.opencode/plugin/superfastpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "superfastpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > superfastpowers)
          const resolved = skillsCore.resolveSkillPath(
            skill_name,
            superfastpowersSkillsDir,
            personalSkillsDir
          );
...
Run: `node -c .opencode/plugin/superfastpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the superfastpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const superfastpowersSkills = skillsCore.findSkillsInDir(
            superfastpowersSkillsDir,
            'superfastpowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...superfastpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install superfastpowers skills to ~/.config/opencode/superfastpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'superfastpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-superfastpowers skill content
      const usingSuperfastpowersPath = skillsCore.resolveSkillPath(
        'using-superfastpowers',
        superfastpowersSkillsDir,
        personalSkillsDir
      );

      let usingSuperfastpowersContent = '';
      if (usingSuperfastpowersPath) {
        const fullContent = fs.readFileSync(usingSuperfastpowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingSuperfastpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Superfastpowers skills: \`superfastpowers:skill-name\` (from ~/.config/opencode/superfastpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/superfastpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/superfastpowers && git pull` to update superfastpowers.' :
        '';

      // Return context to inject into session
      return {
        context: `<EXTREMELY_IMPORTANT>
You have superfastpowers.

**Below is the full content of your 'superfastpowers:using-superfastpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingSuperfastpowersContent}
...
```markdown
# Installing Superfastpowers for OpenCode
...
### 1. Install Superfastpowers Skills

```bash
# Clone superfastpowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/superfastpowers
git clone https://github.com/obra/superfastpowers.git ~/.config/opencode/superfastpowers
```

### 2. Install the Plugin

The plugin is included in the superfastpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/superfastpowers/.opencode/plugin/superfastpowers.js`
...
ln -s ~/.config/opencode/superfastpowers/.opencode/plugin/superfastpowers.js .opencode/plugin/superfastpowers.js
```
...
```
use use_skill tool with skill_name: "superfastpowers:brainstorming"
```
...
Personal skills override superfastpowers skills with the same name.

## Updating

```bash
cd ~/.config/opencode/superfastpowers
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/superfastpowers/.opencode/plugin/superfastpowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/superfastpowers/skills`
...
- Report issues: https://github.com/obra/superfastpowers/issues
- Documentation: https://github.com/obra/superfastpowers
```
...
Superfastpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
**Files:**
- Test: `.codex/superfastpowers-codex`
...
Run: `.codex/superfastpowers-codex use-skill superfastpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/superfastpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/superfastpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/superfastpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/superfastpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/superfastpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/superfastpowers.js` created with tools and hooks
